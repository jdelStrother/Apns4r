...
module APNs4r
  ...
  class PayloadHash < Hash
    ...
    # Invokes {PayloadHash#to_s} and returns it's length
  ...
  class Notification
    ...
    def Notification.create(token, payload)
      Notification.new token.kind_of?(String) ? token.delete(' ') : token.to_s(16) , payload.kind_of?(Hash) ? PayloadHash.new(payload).to_s : payload
    end
